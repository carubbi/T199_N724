...
**Há associação observacional entre baixo desempenho em quadra e não permanência observada no mesmo time na temporada seguinte?**
...
- criar os indicadores de baixo desempenho usados em $X$;
...
$$
X = \text{número de indicadores de baixo desempenho observados no caso jogador-temporada}
$$
...
em que $B$ representa baixo desempenho.
...
- o que $X$ mostra sobre o perfil de baixo desempenho;
...
- **04/05/2026 - Apresentação do trabalho e Preparação dos dados.** Apresentação do enunciado, da pergunta de pesquisa, das variáveis obrigatórias, da estrutura mínima do notebook e dos critérios de avaliação. A dupla deverá iniciar a preparação dos dados, incluindo carregamento do dataset, criação de `jogador_temporada` e organização das colunas necessárias.
- **06/05/2026 - Associação quantitativa e revisão para AV2.** A dupla deverá concluir a análise com correlação/covariância, matriz ou heatmap e interpretação da associação entre `pontos`, `minutos_totais`, `jogos_disputados` e `salario_usd`. Aula de revisão dos conceitos centrais da Unidade 2 e esclarecimento de dúvidas para a AV2.
...
- **14/05/2026 - Entrega do trabalho.** Entrega do notebook documentado, executável e com interpretações curtas, além do vídeo explicativo com síntese dos resultados e limitações.
...
- **Variável aleatória discreta $X$ (0,20 ponto):** construção dos indicadores de baixo desempenho, PMF, CDF, média, desvio padrão e interpretação.
